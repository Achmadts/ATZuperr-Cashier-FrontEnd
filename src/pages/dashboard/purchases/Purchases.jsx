...
const PurchasesTable = () => {
  ...
  const fetchPurchases = async () => {
    try {
      ...
      const response = await fetch(
        `${endpoints.purchase}?page=${currentPage}&per_page=${showEntries}&searchTerm=${searchTerm}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleDeletePurchase = async (id) => {
    try {
      ...
      console.error(error);
      showToast("Terjadi kesalahan saat menghapus data pembelian.", "error");
    }
  };
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      const response = await fetch(endpoints.purchaseExport, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const openModalWithPurchase = (purchase) => {
                  ...
                  : purchases.map((purchase) => (
                      <tr key={purchase.id}>
                        ...
                        <td>{`Rp. ${new Intl.NumberFormat("id-ID", {
                          ...
                          .format(purchase.total_pembayaran)
                          .replace("Rp", "")
                          .trim()}`}</td>
                        <td className="border border-gray-300 px-4 py-2">
                        ...
                        <td className="border border-gray-300 px-4 py-2">
                          <div className="md:hidden">
                            <button
                              ...
                              onClick={() => openModalWithPurchase(purchase)}
                            >
                              Actions
                            </button>
                          </div>
                          ...
                          >
                            ...
                            <div className="flex flex-col space-y-2">
                              <button
                                ...
                                onClick={() => {
                                  if (selectedPurchase) {
                                    navigate(`edit/${selectedPurchase.id}`);
                                    console.log("ID: " + selectedPurchase.id);
                                  }
                                  closeModal(true);
                                }}
                              >
                              ...
                              <button
                                ...
                                onClick={() => {
                                  if (selectedPurchase) {
                                    navigate(`details/${selectedPurchase.id}`);
                                    console.log("ID: " + selectedPurchase.id);
                                  }
                                  closeModal(true);
                                }}
                          ...
                          <div className="hidden md:flex space-x-1 justify-center items-center">
                            ...
                            <button
                              ...
                              onClick={() => navigate(`details/${purchase.id}`)}
                            >
                              Details
                              <InfoOutlined
                            ...
                            <button
                              ...
                              onClick={() => openModalActionDelete(purchase.id)}
                            >
                              Delete
